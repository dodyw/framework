...
class DBWrapperPearDB {
  ...
  function DBWrapperPearDB( &$db ) {
    ...
    if ( is_subclass_of( $db, 'db_common' ) ) {
      ...
      die( CF_ERR_DBWRAPPER_NOT_PEARDB );
  }
  ...
  function execute( $sql ) {
    ...
    if ( !PEAR::isError( $result ) ) {
      $rsobject = new DBWrapperPearDBResult( $result, $this->link );
      $rsobject->moveNext();
      return $rsobject;
    }
    else {
  ...
  function quote( $string, $quotes = false ) {
    return $this->link->quoteSmart( ( $quotes )? stripslashes( $string ) : $string );
  }
...
class DBWrapperPearDBResult {
  ...
  /**
   * Constructor: holds a PEAR DB result object
   *
   * @param object PEAR DB result object
   * @param object PEAR DB database object
   */
  function DBWrapperPearDBResult( &$result, $link ) {
    $this->result = &$result;
  }
